...
export const Navbar = () => {
    ...
    return (
        <div className='z-50 fixed flex justify-center w-full text-white font-bold'>
            ...
            <div onClick={toggleNav} className='md:hidden fixed top-5 right-14 border rounded z-50 text-white/70 border-white/70 p-2 '>
            ...
            >
                <ul className='flex flex-col items-center justify-center space-y-8 h-full'>
                    {navLinks.map((link, index) => (
                        <li key={index}>
                            <Link href={link.path} onClick={closeNav} className='transform hover:text-white/50 transition-all duration-300 ease-in-out text-2xl text-center'>
                                {link.title}
                            </Link>
                        </li>
                     ))}
